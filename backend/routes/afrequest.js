...
async function handleAcceptRequest(req, res) {
  try {
    ...
    // Lấy thông tin người gửi + username của receiver
    ...
    // Thêm vào bảng Friend (đảm bảo user1_id < user2_id để tránh trùng lặp)
    const [user1, user2] = senderId < currentUserId
      ? [senderId, currentUserId]
      : [currentUserId, senderId];

    await pool.request()
      .input('user1_id', sql.Int, user1)
      .input('user2_id', sql.Int, user2)
      .query(`
        IF NOT EXISTS (
          SELECT * FROM Friend WHERE user1_id = @user1_id AND user2_id = @user2_id
        )
        BEGIN
          INSERT INTO Friend (user1_id, user2_id) VALUES (@user1_id, @user2_id)
        END
      `);

    // Tạo notification cho sender
    const notificationContent = `${receiverUsername} đã đồng ý yêu cầu kết bạn của bạn`;
    ...
    res.status(200).json({ message: 'Yêu cầu đã được chấp nhận, thêm bạn bè và tạo thông báo cho sender' });
  } catch (err) {
